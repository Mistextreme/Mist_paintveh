...
InitializePaint = function(pos)
    ...
    local input = lib.inputDialog('Paint Job', {
        ...
        { type = 'color', label = 'Colour', default = '#ffffff', required = function() return input[1] ~= 'chameleon' end }
    })
    ...
    if input[1] == 'chameleon' then
        SelectChameleonColor(pos, vehicle)
        return
    end
...
function SelectChameleonColor(pos, vehicle)
    -- Salva as cores originais para possível restauração
    originalVehicleColors.primary, originalVehicleColors.secondary = GetVehicleColours(vehicle)
    originalVehicleColors.pearl, originalVehicleColors.wheel = GetVehicleExtraColours(vehicle)

    local options = {}
    for _, color in ipairs(Config.ChameleonColors) do
    ...
    local input = lib.inputDialog('Chameleon Colors', {
        {
            type = 'select',
            label = 'Select Chameleon Color',
            options = options,
            required = true
        },
        {
            type = 'checkbox',
            label = 'Preview color before buying ($'..Config.PreviewCost..')',
            default = true
        }
    })
    ...
    local colorId = input[1]
    local wantsPreview = input[2]

    if wantsPreview and Config.EnablePreview then
        StartChameleonPreview(pos, vehicle, colorId)
    else
        ApplyChameleonPaint(pos, vehicle, colorId)
    end
end

function StartChameleonPreview(pos, vehicle, colorId)
    previewMode = true
    previewVehicle = vehicle
    
    -- Aplica a cor temporária
    SetVehicleExtraColours(vehicle, colorId, 0)
    FreezeEntityPosition(vehicle, true)

    -- Mostra instruções interativas
    local startTime = GetGameTimer()
    local colorData = Config.ChameleonColors[colorId - 157]
    local totalPrice = colorData.price + Config.PreviewCost

    lib.showTextUI(Config.PreviewMessage:format(totalPrice), {
        position = 'top-center',
        icon = 'spray-can'
    })

    -- Thread de controle do preview
    Citizen.CreateThread(function()
        while previewMode do
            ...
            elseif IsControlJustPressed(0, Config.CancelKey) then
                ...
                _ShowNotification(Config.PreviewCancelled)
                previewMode = false
                break
            end

            -- Tempo limite
            if (GetGameTimer() - startTime) > (Config.PreviewTime * 1000) then
                lib.hideTextUI()
                ResetVehicleColors(vehicle)
                _ShowNotification("~o~Preview time expired")
                previewMode = false
                break
            end
...
function ApplyChameleonPaint(pos, vehicle, colorId)
    previewMode = false
    local colorData = Config.ChameleonColors[colorId - 157]
    local totalPrice = colorData.price
    
    -- Se estava em preview, adiciona custo extra
    if previewVehicle == vehicle then
        totalPrice = totalPrice + Config.PreviewCost
    end

    -- Confirma a aplicação da cor
    SetVehicleExtraColours(vehicle, colorId, 0)
    TriggerServerEvent('bryan_paintjob:server:pay', totalPrice)
    _ShowNotification(string.format(Config.ColorApplied, totalPrice))

    -- Reset
    previewVehicle = nil
    originalVehicleColors = {}
end

function ResetVehicleColors(vehicle)
    SetVehicleColours(vehicle, originalVehicleColors.primary, originalVehicleColors.secondary)
    SetVehicleExtraColours(vehicle, originalVehicleColors.pearl, originalVehicleColors.wheel)
end
    ...
    if resourceName == GetCurrentResourceName() then
        ...
        if previewMode then
            ...
            if previewVehicle then
                ResetVehicleColors(previewVehicle)
